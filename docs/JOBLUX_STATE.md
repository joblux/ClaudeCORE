...
**Next step: Pack B.3.2 — member lifecycle write path (resolver guard + delete endpoint + Settings UI scaffold).**

Substrate from B.3.1 is fully defended at DB layer but invisible to the app — no code reads `members.deleted_at` yet. B.3.2 is the first slice that turns the substrate on at runtime. Scope (locked at B.3 audit, awaiting Mo direction to start): (1) Resolver guard — `resolveProfiLux` returns `null` when `deleted_at IS NOT NULL`; cascades hide through all 6 projections automatically. (2) `/api/members/delete` endpoint — auth-required, sets `deleted_at = now()` + `deleted_by = <self>`, forces `share_links.sharing_enabled = false` server-side, hard-deletes `nextauth_accounts` rows for the member, revokes active session. (3) Admin variant — `/api/admin/members/[id]` delete with `deleted_by = <admin_id>` + restore action (clear `deleted_at`). (4) Admin Trash UI — `/admin/trash` or `/admin/members?filter=deleted` listing + restore. (5) Candidate Settings page — greenfield `/dashboard/candidate/settings` hosting the delete-account control + future matching_opt_in (B.3.3) + future RGPD export trigger (B.3.4). NOT a Manage extension per MATRIX §19.4 + §21.3. Doctrine reference: `docs/PROFILUX_MATRIX_V1.md` §25.

**Queued after B.3.2:** Pack B.3.3 — `matching_opt_in` substrate + endpoint + UI + §20 reopen + View tab caption decision (gate vs remove). Pack B.3.4 — RGPD export endpoint + UI + MATRIX §19B addition.

**Remaining Pack B (later):** masked_fields consumers, ProfiLux PDF export, slug rotation/history UI, share view tracking consumer, full `profilux` table retire (ledger `6aef236e` DDL drop).

**After Pack B:** Pack D (Add Section library) → Pack E (Recruiting loop).

**Workflow doctrine locked 2026-05-16, reconfirmed 2026-05-17:** Claude AI = audit / scope / QA. GPT = final compact Claude Code prompt. Claude Code = execution. Claude AI does NOT draft Claude Code prompts directly.

**Handoff doc:** `docs/HANDOFF_2026-05-17.md`.
...
**Last updated:** May 17, 2026 (Pack B.1.4 legacy share cleanup shipped at `a5a4bc2` with 9/9 Chrome MCP QA; Pack B.3.1 member lifecycle substrate shipped at `ccc8a6d` docs + `1b4b8c3` DDL with prod migration applied; 2 ledger ops). Total this rotation: 16+ commits across Pack A / F / C / P1 / B.1 / B.1.4 / B.3.1. Next: Pack B.3.2 — resolver guard + delete endpoint + Settings UI scaffold.
...
- Member lifecycle (locked May 16/17 2026 via MATRIX §25): soft-delete shape `members.deleted_at TIMESTAMPTZ NULL` + `members.deleted_by UUID NULL` shipped 2026-05-17 via migration `20260517_member_soft_delete` (DB version `20260516154744`, repo `1b4b8c3`). RLS `Members read own` + `Members update own` policies tightened in the same slice — both now require `deleted_at IS NULL` in USING; `Members update own` also enforces it WITH CHECK. Admin and service-role policies untouched per §25.3 (restore + audit). Email uniqueness migrated to partial unique index `idx_members_email_active` on `lower(email)` WHERE `deleted_at IS NULL`. Resolver `resolveProfiLux` will return `null` when `deleted_at IS NOT NULL` once B.3.2 wires the guard — substrate is DB-defended today but app-invisible until B.3.2 ships. No code path may hard-delete `members` rows. Audit trail preserved by row retention; `nextauth_accounts` + active share_links suppression handled by B.3.2 delete endpoint. Account-level controls belong to Settings, not ProfiLux Manage.
