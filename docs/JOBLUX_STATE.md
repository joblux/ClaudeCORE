...
Phase 4 spec foundation — §7.6 EditorView addendum SHIPPED (commit 917e6dc, 2026-05-02). Phase 4.A implementation in progress; Screens 3, 4, and 8 write-enabled and validated.

1. **Phase 4.A — Next write-enabled screen selection** *(ledger `8f82b3ac`, status: open)* — Screens 3 (Current Position), 4 (Luxury Fit), and 8 (Clienteling) write-enabled, browser + DB validated end-to-end. POST contract validated against §4.5 W1/W2/W3 (empty-string coercion for strings, partial-body, completeness recompute). Vocabulary now consumed by 2 screens: `PROFILUX_SENIORITY_OPTIONS` (Screen 3), `PROFILUX_PRODUCT_CATEGORY_OPTIONS` + `PROFILUX_EXPERTISE_TAG_OPTIONS` (Screen 4). Two UI patterns established and ready for reuse: chip multi-toggle (Screens 7, 9) and tri-state Yes/No chip pair (Screen 9 `open_to_relocation`). Next decision: select which screen receives write/input UI next. No implementation yet. Per Phase 4.A.5 prep inventory, next-easiest unblocked candidates are Screen 6 (Education L2 fields: university, field_of_study, graduation_year — 3 scalar inputs, no vocabulary) and Screen 10 (Salary — 3 fields, currency vocabulary decision required). Sequence after selection: write UI on chosen screen → extend write coverage → 4.B deprecate `toLegacyProfile` → 4.C migrate consumers (3 files / 4 fetch sites) → 4.D remove `toLegacyProfile` → 4.E `/api/members/me` cutover. `normalizeAvailability` / `denormalizeAvailability` retained per D2 + Phase 4.1.
...
**Last updated:** May 4, 2026 (Phase 4.A.5 Screen 8 write-enabled, browser+DB validated — `5668d61`)
