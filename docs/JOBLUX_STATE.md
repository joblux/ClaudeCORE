...
- **6d820f7** `fix(cv-parse): recompute profile_completeness after L1 write (D2)` — May 9 2026 AM. SHIPPED + PROD-VALIDATED 4/4. Single-file fix (`app/api/members/cv-parse/route.ts`, +26/-0). Closes Matrix v1 §4.4 violation: `/api/members/cv-parse` wrote `cv_parsed_data` + `cv_parsed_at` but did NOT recompute `profile_completeness`. Fix: after the primary `cv_parsed_data` UPDATE and success `logHistory`, resolve the post-write member view via `resolveProfiLux(memberId, supabase)`, recompute via `computeProfileCompleteness(view)`, and persist the score if it moved. Three contract decisions (locked by GPT): recompute is SECONDARY consistency side-effect, resolver/scorer failures are NON-FATAL; NO second `updated_at` bump on the score-only UPDATE; cached early-return path SKIPS recompute. Prod QA: 4/4 PASS.

- **392c947** `fix(members-profile): remove legacy profile_completeness recompute (D3 option beta)` — May 9 2026 AM. SHIPPED + PROD-VALIDATED 5/5. Single-file fix (`app/api/members/profile/route.ts`, +1/-56). Closes the triple-system completeness drift on the legacy route. Per GPT decision (Option β): the route should NOT touch `profile_completeness` at all — it is identity/location/account writer for business members, not a ProfiLux/matching-readiness writer. Fix: deleted the in-file `calculateProfileCompleteness` async helper, removed post-update re-fetch + recompute + second UPDATE, dropped `profile_completeness` from PUT/POST response body. Definitive DB proof — pre-fix legacy scorer would have written 15 → 10 after phone clear; actual DB shows `profile_completeness = 15` UNCHANGED → legacy recompute path is dead. Prod QA: 5/5 PASS.
...
- Mix Operator Bridge / Away Mode / deploy reconciliation work into ProfiLux feature sessions. Bridge/Away Mode is a separate workflow-infrastructure track; trigger phrase: *"Open JOBLUX workflow infrastructure session — Operator Bridge / Away Mode"*.
...
**Last updated:** May 9, 2026 — End of session. Two slices shipped this session: Operator Bridge v0 skills at `e43c2fc` (3 SKILL.md files, doc-as-code workflow templates, no autonomy) + Manage tab v0 at `a829033` (read-only Visibility & sharing panel, A1 refined, 10/10 PROD-VALIDATED). `/joblux-open` and `/joblux-review` validated this session; `/joblux-close` PARTIALLY validated — produced all artifacts correctly but artifact transfer friction remains. Two new findings logged from the close attempt: `F-close-skill-artifact-friction` (sandbox paths vs local repo) + `F-github-mcp-write-scope-blocked` (direct push attempt failed 403, write scope unavailable). Workaround applied: heredoc-embedded full artifact content in a single Claude Code prompt — Mo pastes once, Claude Code writes both files locally + commits + pushes. Coolify deploy of `cc0f954` failed but was bypassed by later successful deploys; logged as `F-coolify-failed-deploy-orphan`. Bridge/Away Mode work is a SEPARATE workflow-infrastructure track, NOT mixed into ProfiLux feature sessions; trigger phrase: *"Open JOBLUX workflow infrastructure session — Operator Bridge / Away Mode"*. V1 priorities for that track: artifact handoff bridge (eliminates heredoc pattern), GitHub MCP write scope OR Claude-Code-as-artifact-bridge, deploy reconciliation logic, `/joblux-close` real validation, Agent SDK exploration. ProfiLux doctrine reaffirmed: View = living document, Edit = enrichment, Manage = visibility/use status (read-only this slice). Legacy `profilux` table stays isolated from EditorView/resolver/projectFor; share state read via dedicated `/api/profilux/share` endpoint only. HEAD `a829033`. Next recommended product slice: **B (Identity micro-additions)** unless Mo overrides.
