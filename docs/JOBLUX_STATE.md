...
Phase 4 spec foundation — §7.6 EditorView addendum SHIPPED (commit 917e6dc, 2026-05-02). Phase 4.A implementation in progress; Screens 3, 4, 6, 8, and 10 write-enabled and validated.

1. **Phase 4.A — Next write-enabled screen selection** *(ledger `8f82b3ac`, status: open)* — Screens 3 (Current Position), 4 (Luxury Fit), 6 (Education L2 fields), 8 (Clienteling), and 10 (Salary) write-enabled, browser + DB validated end-to-end. POST contract validated against §4.5 W1/W2/W3 (empty-string coercion for strings, partial-body, completeness recompute). Range guard pattern established for Screen 10 min/max (400 server-side rejection, no silent swap). Vocabulary now consumed by 3 screens: `PROFILUX_SENIORITY_OPTIONS` (Screen 3), `PROFILUX_PRODUCT_CATEGORY_OPTIONS` + `PROFILUX_EXPERTISE_TAG_OPTIONS` (Screen 4), `PROFILUX_CURRENCY_OPTIONS` (Screen 10). Three UI patterns established: chip multi-toggle (Screen 4, reusable for 7/9), tri-state Yes/No chip pair (Screen 8, reusable for 9 `open_to_relocation`), number-pair + select (Screen 10, reusable for 9 if any range fields). **GPT-locked next:** Screen 9 (Availability + targets) — 6 fields, denormalize-on-write rule + locations vocabulary decision. Screen 7 (Skills + markets — needs new vocabularies `PROFILUX_SKILL_OPTIONS` + `PROFILUX_MARKET_OPTIONS`, multi-commit structure like Phase 4.A.4) deferred until after Screen 9. Screen 5 (Career history — read-only L1 only, no write surface in v1) and Screen 11 (Confirm — separate `m6_confirmed_at` route, out of scope for write-enable rollout) excluded. Sequence after Screen 9 lands: Screen 7 → 4.B deprecate `toLegacyProfile` → 4.C migrate consumers (3 files / 4 fetch sites) → 4.D remove `toLegacyProfile` → 4.E `/api/members/me` cutover. `normalizeAvailability` / `denormalizeAvailability` retained per D2 + Phase 4.1.
