...
1. **Phase 4.B — `toLegacyProfile` deprecation** *(ledger `8f82b3ac`, status: open)* — Read-only audit first. The `toLegacyProfile(view)` adapter in `app/api/profilux/route.ts` (line ~108) still produces the lossy `.profile` envelope returned alongside `.editor` and `.view` in GET/POST responses. One real consumer (audit-verified, May 5 2026): `app/dashboard/candidate/page.tsx`. `/api/members/me` does NOT depend on `toLegacyProfile` (uses `toLegacyMember` from `ProfiLuxResolved`). Business dashboard and ProfileClient do not read `data.profile`. Audit goal: enumerate all consumers reading `data.profile` (vs `data.editor`/`data.view`), document field-by-field which legacy fields each consumer needs, identify what breaks if `toLegacyProfile` is removed. **No implementation in this audit.** No deletes, no consumer migration. Output: a finding-style summary documenting consumer dependency map. Sequence after audit: Phase 4.C (migrate consumers to `data.editor`) → Phase 4.D (remove `toLegacyProfile` from route) → Phase 4.E (`/api/members/me` cutover).
