...
- **F-business-fields-untyped** *(NEW 2026-05-09c, observation_only)* — `(member as any).company_name` + `(member as any).org_type` casts in `app/admin/members/[id]/page.tsx` access fields not present on the canonical `members.*` object. Likely a `business_profiles` join or untyped columns. Casts preserved verbatim at F-1a (`3edf6ac`) intentionally — resolution scoped to F-2 (separate slice). NOT a bug; tracking only so F-2 has a forward pointer.
...
**Last updated:** May 9, 2026 (c) — End of session. One refactor slice shipped: F-1a at `3edf6ac` (`refactor(profilux): kill MemberProfile, formalize as AdminMemberDetail adapter`). Eliminates the parallel `MemberProfile` type system in `types/member-profile.ts` (114 lines, deleted) consumed only by `app/admin/members/[id]/page.tsx`; replaces with canonical `AdminMemberDetail = Omit<ProfiLuxResolved, 'languages'> & { full_name, notes, work_experiences[], education_records[], languages[], documents[] }` adapter type in `lib/profilux/types.ts` plus 4 sub-types (`AdminWorkExperience`, `AdminEducationRecord`, `AdminLanguage`, `AdminMemberDocument`). 5 files, +112/-142 net. Build green on 2nd pass after one bounded type-only fix (Omit on `'languages'` to break TS intersection collision); all 5 conditions of Step 8 corrective-pass envelope satisfied. Prod-validated 7/7 on Alex Mason fixture `e6899932` (all admin detail tabs render). `(member as any).company_name` + `.org_type` casts preserved as-is for F-2 (business member type reconciliation, separate slice). Doctrine added: (1) `AdminMemberDetail` is the ONE place `ProfiLuxResolved` is extended for an admin client surface — sibling adapters required for other surfaces; (2) `Omit<ProfiLuxResolved, 'languages'>` pattern formalized for adapters synthesizing stable IDs on L1 passthrough arrays; (3) `C-B-2` (admin share-preview) parked pending UX/product decision on client-template visual; (4) parallel-client-type systems for admin surfaces are dead. Findings logged: `F-r-slug-local-types-redeclared` (observation only — public surface mirror of the killed admin pattern; defer to `C-B-3`), `F-business-fields-untyped` (observation only — F-2 forward pointer), `F-admin-detail-omit-pattern` (doctrine note). PARKED additions: `F-2`, `C-B-2`, `C-B-3`. Bridge V1.1 second real-use close = THIS close (2026-05-09c); friction metrics quiet (no heredoc, no Downloads, no fallback). Coolify deploy on `3edf6ac`: `git_only` by design — Mo to verify dashboard green; real verification still awaits the `F-runtime-build-sha-not-exposed` slice. All product unresolveds carry forward unchanged from prior HANDOFF (`90567a0`). HEAD `3edf6ac`. Next product slice: **undecided — F-2 (business member type reconciliation, technical debt) or wait for client-template visual lock to unblock C-B-2 (admin share-preview)**, Mo + GPT pick.
