...
- **c449cd1 + Supabase migration `f_2_4_drop_members_dead_company_columns`** `refactor(admin): F-2-4 swap dead members.company_* fields for company_name + org_type in ai-review prompt + DDL drop` — May 10 2026 (e). SHIPPED + COOLIFY-GREEN + DDL-APPLIED. F-2-4 launch-cleanup slice closed end-to-end across 4 gates. **Commit 1 (code) `c449cd1`**: 1 file, +2/-3, `app/api/admin/members/ai-review/route.ts` lines 79-81. The 3 dead `members.company_*` prompt fields (`company_email`, `company_website`, `company_size`, all 0/12 populated per Gate 3 pre-check) were always feeding `'N/A'` into the Haiku ai-review prompt; replaced with `members.company_name` + `members.org_type`, both already on the member object via existing `select('*')` (no SELECT broadening, no new query, no other file touched). Live values surfaced to admin LIST at `e4fdf46` and admin DETAIL at `b982f53`. Net behavioral change: Haiku ai-review prompt loses 3 always-N/A lines for business members, gains 2 actual-data lines (`company_name`, `org_type`); professional members unchanged (still N/A — fields not applicable). **Commit 2 (DDL) Supabase migration `f_2_4_drop_members_dead_company_columns`**: `ALTER TABLE members DROP COLUMN company_email; DROP COLUMN company_website; DROP COLUMN company_size;`. No rollback migration. Verified post-migration via `information_schema.columns` query — 0 of the 3 columns remain on `members`. **Validation gates:** GATE 1 push exit 0 (c449cd1 on main, GitHub MCP confirmed file SHA `68e88ab`) ✓; GATE 2 Coolify green (Mo visual confirmation on `c449cd1`) ✓; GATE 3 Supabase pre-check (0/12 populated across all 3 columns, exact match to STATE F-2-4 claim) ✓; GATE 4 DDL `apply_migration` success + `information_schema` verification empty ✓. **Out-of-scope finding logged separate**: `F-stale-schema-artifacts` (`supabase-schema.sql` + `types/database.ts` both describe a dead pre-migration `profiles` table — discovered during F-2-4 scan, contradicts STATE §6 single-source-of-truth claim, not in this slice's scope). STATE-only slice-end closure applied (no HANDOFF V3, no doctrine additions, no DO NOT changes). admin_tasks ledger reconciliation deferred to `/joblux-close` per Mo instruction. **STATE write path note:** GitHub MCP `create_or_update_file` failed mid-session (manifested `F-github-mcp-write-scope-blocked`); fallback path = Claude Code local surgical edit + commit, this entry written via that path.
...
F. **Launch checklist items:** b7590e0d (logged-in audit), 8651a836 (Resend/magic-link), 7becdb12 (compliance pack), 6f57a924 (private-layer noindex), 04c65c54 (hardcoded numbers).

**Recommended product order on resume: E (small B-series follow-ups in batches) → A2 → C → D.** B-series are small hardening slices in the proven B19-followup shape; A2 unparks reset-link; C pivots to public projection convergence; D is post-launch.
...
**Last updated:** May 10, 2026 (e) — F-2-4 dead schema cleanup shipped end-to-end (`c449cd1` code + Supabase migration `f_2_4_drop_members_dead_company_columns`). 4-gate validation (push / Coolify / DB pre-check / DDL apply) all PASS. STATE-only slice-end closure applied (admin_tasks reconciliation deferred to `/joblux-close`). New finding logged: `F-stale-schema-artifacts` (out-of-scope dead schema files discovered during F-2-4 scan). HEAD `c449cd1` + Supabase migration applied. Session continues.
