...
export async function checkDuplicate(
  ...
  switch (payload.content_type) {
    case 'event':
      return checkEvent(payload, currentQueueId)
    case 'signal':
      return checkSignal(payload, currentQueueId)
    case 'article':
      return checkArticle(payload, currentQueueId)
    case 'salary_benchmark':
      return checkSalary(payload, currentQueueId)
    default:
      return NO_MATCH
  }
}
...
async function checkEvent(
  ...
  if (p.start_date && p.city) {
    ...
    const { data } = await supabaseAdmin
      .from('events')
      .select('id, title, name, city, location_city')
...
async function checkSignal(
  ...
  if (p.source_url) {
    ...
    if (data?.[0]) {
      return hit(data[0].id, data[0].headline, 'signal', 'published', 'signals')
    }
  }
  ...
  if (p.source_url) {
    ...
    if (data?.[0]) {
      return hit(data[0].id, data[0].title, 'signal', data[0].status, 'content_queue')
    }
  }
...
async function checkArticle(
  ...
  if (p.slug) {
    ...
    if (data?.[0]) {
      return hit(data[0].id, data[0].title, 'article', 'published', 'bloglux_articles')
    }
  }
...
async function checkSalary(
  ...
  if (p.job_title && p.city && p.seniority) {
    ...
    const { data } = await supabaseAdmin
      ...
      .select('id, brand_slug, job_title, city, seniority')
      .limit(1000)
    ...
    for (const r of data || []) {
      if (
        ...
        return hit(
          r.id,
          `${r.brand_slug} — ${r.job_title} (${r.city})`,
          'salary_benchmark',
          'published',
          'salary_benchmarks'
        )
