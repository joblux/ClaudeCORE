-- 20260525_cv_merge_apply_preserve_cv_parsed_data.sql
-- S-C (preserve-old-only): apply_cv_merge no longer wholesale-replaces
-- members.cv_parsed_data with the new pending blob. The prior swap dropped any
-- field the old parse held and the new parse lacked (completeness regression)
-- and would silently import new unaccepted CV data into the live L1 fallback.
-- Accepted items still write to L2 (identity UPDATE + collection INSERTs).
-- Now the final step ONLY clears cv_parsed_pending; cv_parsed_data/cv_parsed_at
-- are preserved. p_new_cv_parsed_data is intentionally unused (kept in the
-- signature so the calling route needs no change). Already live in prod.

CREATE OR REPLACE FUNCTION public.apply_cv_merge(p_member_id uuid, p_accept jsonb, p_new_cv_parsed_data jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
...
BEGIN
  ...
  -- S-C preserve-old-only: clear pending ONLY. Do not touch cv_parsed_data.
  UPDATE public.members SET
