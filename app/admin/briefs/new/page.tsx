...
// ── Form state type (covers every JobBrief field we can edit) ────────
...
function NewBriefPage() {
  ...
  // ── Fetch existing brief for edit mode ───────────────────────────
  useEffect(() => {
    if (!editId) return

    setLoading(true)
    fetch(`/api/briefs/${editId}`)
      .then((res) => {
      ...
      .then((data) => {
        ...
        setForm({
          title: brief.title || '',
          maison: brief.maison || '',
          is_confidential: brief.is_confidential || false,
          status: brief.status || 'draft',
          source: brief.source || 'Manual Entry',
          priority: brief.priority || 'normal',
          assigned_recruiter: brief.assigned_recruiter || '',

          city: brief.city || '',
          country: brief.country || '',
          remote_policy: brief.remote_policy || '',
          relocation_offered: brief.relocation_offered || false,
          visa_sponsorship: brief.visa_sponsorship || false,

          department: brief.department || '',
          seniority: brief.seniority || '',
          contract_type: brief.contract_type || '',
          reports_to: brief.reports_to || '',
          team_size: brief.team_size != null ? String(brief.team_size) : '',
          start_date: brief.start_date || '',

          salary_min: brief.salary_min != null ? String(brief.salary_min) : '',
          salary_max: brief.salary_max != null ? String(brief.salary_max) : '',
          salary_currency: brief.salary_currency || 'EUR',
          salary_period: brief.salary_period || 'Annual',
          salary_display: brief.salary_display || '',
          bonus_commission: brief.bonus_commission || '',
          ...
          about_maison: brief.about_maison || '',

          product_category: brief.product_category || [],
          client_segment: brief.client_segment || '',
          languages_required: brief.languages_required || [],
          clienteling_experience: brief.clienteling_experience || false,
          travel_percentage: brief.travel_percentage || '',
          luxury_sector_experience: brief.luxury_sector_experience || '',
          ...
          client_contact_name: brief.client_contact_name || '',
          client_contact_email: brief.client_contact_email || '',
          client_contact_phone: brief.client_contact_phone || '',
          fee_agreement: brief.fee_agreement || '',
          fee_amount: brief.fee_amount || '',
  ...
  const updateField = useCallback(
    (field: keyof BriefFormState, value: string | boolean | string[]) => {
  ...
  useEffect(() => {
    ...
    setForm((prev) => {
      const updates: Partial<BriefFormState> = {}
  ...
  const handleSubmit = async (submitStatus: 'draft' | 'published') => {
    ...
    try {
      const url = isEditMode ? `/api/briefs/${editId}` : '/api/briefs'
      ...
      if (!res.ok) {
        ...
        throw new Error(data.error || `Failed to ${isEditMode ? 'update' : 'save'} brief`)
      }
  ...
  return (
    <main className="min-h-screen bg-[#f5f5f5]">
      ...
      <section className="border-b border-[#e8e8e8] bg-[#f5f5f5]">
        <div className="max-w-3xl mx-auto px-6 py-8">
          ...
          <h1 className="jl-serif text-2xl md:text-3xl text-[#1a1a1a] mt-4">
            {isEditMode ? 'Edit Job Brief' : 'New Job Brief'}
          </h1>
          <p className="text-sm text-[#666] mt-1">
            {isEditMode
              ? 'Update assignment details for this brief.'
              : 'Create a new assignment for the JOBLUX platform.'}
          </p>
        </div>
      </section>
      ...
      <div className="max-w-3xl mx-auto px-6 py-8">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <p className="text-xs text-[#999] mt-1">
                Type to search or enter any brand name. Use &quot;Confidential Maison&quot; for discreet briefs.
              </p>
            </div>
        ...
        {/* ════════════════════════════════════════════════════════════
            SECTION 8 | Internal / Admin (starts CLOSED)
        ════════════════════════════════════════════════════════════ */}
        <FormSection title="Internal / Admin" defaultOpen={false}>
          <div className="space-y-4">
            {/* Client Contact */}
