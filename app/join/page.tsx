  ...
  if (status === "loading" || status === "authenticated") {
    return (
      <div className="flex items-center justify-center py-12">
        <div className="w-5 h-5 border border-[#e8e6df] border-t-[#a58e28] rounded-full animate-spin" />
      </div>
    );
  }
    ...
    <>
      ...
      <div className="relative my-6">
        ...
        <div className="relative flex justify-center text-xs">
          <span className="bg-white px-4 text-[#999] uppercase tracking-widest">or</span>
        </div>
      </div>

      {emailSent ? (
        <div className="text-center py-4">
          <div className="w-12 h-12 mx-auto mb-4 rounded-full bg-[#f5f4f0] flex items-center justify-center">
          ...
          <p className="text-sm text-[#1a1a1a] font-medium mb-1">Check your inbox</p>
        ...
        <form onSubmit={handleMagicLink}>
          ...
          <input
            ...
            className="w-full px-4 py-3 text-sm border border-[#e8e6df] rounded-sm bg-white text-[#1a1a1a] placeholder:text-[#ccc] focus:outline-none focus:border-[#a58e28] transition-colors"
          />
          <button type="submit" disabled={isSubmitting}
            className="w-full mt-3 px-4 py-3 bg-[#1a1a1a] text-white text-sm font-medium rounded-sm hover:bg-[#333] transition-colors disabled:opacity-50">
    ...
    <main className="min-h-screen bg-[#f5f4f0] flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-[420px]">
        ...
        <div className="bg-white border border-[#e8e6df] rounded-sm p-8">
          <h2 className="text-2xl text-[#1a1a1a] mb-1 text-center" style={{ fontFamily: "'Playfair Display', Georgia, serif" }}>
            Request Access
          </h2>
          <p className="text-sm text-[#777] text-center mb-8">Luxury intelligence, decoded.</p>
        ...
        <p className="text-center text-xs text-[#999] mt-6 leading-relaxed">
          Already have access? <span className="text-[#1a1a1a]">Sign in above</span> — we&apos;ll take you straight to your dashboard.
        </p>
      </div>
    </main>
  );
