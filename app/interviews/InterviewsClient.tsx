...
interface InterviewsClientProps {
  initialExperiences: InterviewExperienceListItem[]
  initialBrands: { name: string; slug: string }[]
  initialStats: InterviewStats | null
  initialTotal: number
...
export default function InterviewsClient({
  ...
  const [experiences, setExperiences] = useState<InterviewExperienceListItem[]>(initialExperiences)
  const [stats, setStats] = useState<InterviewStats | null>(initialStats)
  const [brands, setBrands] = useState<{ name: string; slug: string }[]>(initialBrands)
  const [uniqueDepartments, setUniqueDepartments] = useState<string[]>(initialDepartments)
  const [uniqueSeniority, setUniqueSeniority] = useState<string[]>(initialSeniority)
  ...
  // Only refetch when page > 1 or filters change (page 1 with no filters uses server data)
  useEffect(() => {
    if (page === 1 && !hasFilters) return
    fetchData()
  }, [fetchData, page, hasFilters])
  ...
  const clearFilters = () => {
    ...
    setUniqueDepartments(initialDepartments)
    setUniqueSeniority(initialSeniority)
  }
      ...
      <div className="max-w-lg mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-12">
          {[
            ...
            { brand: 'Hermes', role: 'Store Manager', dept: 'Store Operations · Senior', rounds: 5, diff: 4 },
