...
export default function SalariesClient({ initialEntries, initialTotal, initialStats, initialFilters }: SalariesClientProps) {
  ...
  return (
    <div className="bg-[#f5f4f0] min-h-screen">
      ...
      <div className="jl-container py-8">
        {activeTab === 'browse' && <BrowseTab accessLevel={accessLevel} initialEntries={initialEntries} initialTotal={initialTotal} initialStats={initialStats} initialFilters={initialFilters} />}
        {activeTab === 'benchmark' && (
...
/* ── BROWSE TAB ─────────────────────────────────────── */
function BrowseTab({ accessLevel, initialEntries, initialTotal, initialStats, initialFilters }: { accessLevel: string; initialEntries: any[]; initialTotal: number; initialStats: any; initialFilters: any }) {
  const [entries, setEntries] = useState<SalaryRangeData[]>(initialEntries as SalaryRangeData[])
  ...
  // Track whether user has changed any filter/page from the initial state
  ...
  const fetchData = useCallback(async () => {
    // Skip the first fetch since we already have initial data
      ...
      <div className="lg:col-span-3">
        {stats && stats.total_data_points > 0 && (
          <div className="flex flex-wrap gap-4 text-xs text-[#999] tracking-wide uppercase mb-6">
            ...
            <span>{stats.unique_brands} {stats.unique_brands === 1 ? 'maison' : 'maisons'}</span>
            <span className="text-[#a58e28]">&middot;</span>
            <span>{stats.unique_cities} {stats.unique_cities === 1 ? 'city' : 'cities'}</span>
          </div>
        )}
